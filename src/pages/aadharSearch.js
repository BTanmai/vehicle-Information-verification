...
function AadharSearch() {
    ...
    return (
        <div style={{display:'flex',flexDirection:'column',placeItems:'center'}}>
            <h1>Welcome</h1>
            <TextField label="Please enter aadhar Number"
             value={uid}
             onChange={(e)=>{setUid(e.target.value)}}
             style={{ width: '80vw', maxWidth: '370px', marginLeft: 'auto', marginRight: 'auto' }} variant="outlined" />
             <Button onClick={fetch_data}  style={{maxWidth:'250px',marginLeft:'auto',color:'white',width:'250px',background:'linear-gradient(to right, #9C27B0, #E040FB)',border:0,marginRight:'auto',height:'50px',borderRadius:'5em',marginTop:'5vh',}}>
                        FETCH DETIALS
             </Button>
             <Dialog open={open} onClose={handleClose} style={{borderRadius:'100px'}} aria-labelledby="form-dialog-title">
        <DialogTitle id="form-dialog-title">Vehicle Detials</DialogTitle>
        <DialogContent>
